...
(function () {
	...
	for (i = 0;i < video.subtitle.length;i++) {
		...
		liElement.innerHTML ='<a href="javascript:;" onclick = "player.seekTo('+video.subtitle[i].start+', true);player.playVideo();  clearInterval(myVar);  myVar = setInterval(function(){updateTime('+video.subtitle[i].end+')}, 500);"><span class="glyphicon glyphicon-play-circle">&nbsp</span></a>'+video.subtitle[i].text;
